...
def home_view(request):
    """Show the homepage with the 3 most recently posted active jobs."""
...
def job_list_view(request):
    """
    Show all active jobs with optional filters.
    Filters are read from GET query parameters (e.g. ?keyword=python&location=NYC).
    """
    ...
    if date_posted:
        now = timezone.now()
        # Map filter option to a date range
        date_filters = {
    ...
    # Paginate: 5 jobs per page
    paginator = Paginator(jobs, 5)
    page_obj  = paginator.get_page(request.GET.get('page'))

    # For job seekers: track which jobs they've already applied to or saved
...
def job_detail_view(request, slug):
    """Show the full details of a single job."""
    job = get_object_or_404(Job.objects.select_related('employer'), slug=slug)

    # Check if the current job seeker has already applied
    applied         = False
    applied_job_ids = []
    if request.user.is_authenticated and request.user.role == 'JOB_SEEKER':
...
def job_create_view(request):
    """Let an employer post a new job listing."""
    ...
    if form.is_valid():
        ...
        # Fallback company name if the employer hasn't set one yet
...
def apply_job_view(request, job_id):
    """Let a job seeker apply to a job using their saved resume."""
    ...
    if request.method == "POST":
        ...
        # Create the application using the resume already on the profile
        ...
        # Send confirmation emails (errors are caught so the app still submits)
...
def applied_jobs_view(request):
    """Show all jobs the current job seeker has applied to."""
    applications = (
        JobApplication.objects
        .filter(applicant=request.user.jobseekerprofile)
...
def save_job_view(request, job_id):
    """Toggle a job as saved/unsaved. Returns JSON so it works via AJAX."""
    ...
    if not created:
        saved_job.delete()  # already saved → unsave it

    return JsonResponse({
        'saved':   created,
        'message': "Job saved!" if created else "Job removed from saved list.",
    })
...
def is_job_saved_view(request, job_id):
    """Return JSON indicating whether the job seeker has saved this job."""
    job      = get_object_or_404(Job, id=job_id)
...
def saved_jobs_view(request):
    """Show all jobs the current job seeker has bookmarked."""
    profile    = request.user.jobseekerprofile
    saved_jobs = (
        SavedJob.objects
        .filter(job_seeker=profile)
...
# ─── Employer: Manage Jobs & Applicants ───────────────────────────────────────

@login_required
@role_required('EMPLOYER')
def employer_jobs_view(request):
    """Show all jobs posted by the current employer with applicant counts."""
    jobs = (
        Job.objects
        .filter(employer=request.user.employerprofile)
        .annotate(app_count=Count('applications'))
        .select_related('category', 'industry')
        .order_by('-created_at')
    )
    paginator = Paginator(jobs, 5)
    return render(request, 'jobs/employer_jobs.html', {
        'page_obj': paginator.get_page(request.GET.get('page')),
    })
...
def view_applicants_view(request, job_id):
    """Show all applicants for a specific job (employer only)."""
...
def all_applicants_view(request):
    """Show all applicants across all of the employer's jobs."""
    applications = (
        JobApplication.objects
        .filter(job__employer=request.user.employerprofile)
...
def update_application_status(request, app_id, status):
    ...
    Only the employer who owns the job can update its applications.
    """
...
def download_resume(request, application_id):
    """
    Let an employer download an applicant's resume file.
    Returns a 403 if the employer doesn't own the job or the file is missing.
    """
